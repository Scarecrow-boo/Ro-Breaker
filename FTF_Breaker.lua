...
local window = OrionLibV2:MakeWindow({
    Title = "Breaker | Flee The Facility",
    SubTitle = "Discord | .gg/DMNXkC6CYB"
})
...
MainTab:AddSection({ Name = "Survivor ( update )" })
...
local toggleAntiFail = ToolsTab:AddToggle({
    Name = "Anti Fail",
    Description = "No more fail on computer"",
    Default = false,
...
local toggleAutoInteract = ToolsTab:AddToggle({
    Name = "Auto Interact",
    Description = "Just a Auto Interact",
    Default = false,
